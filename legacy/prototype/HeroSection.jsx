// HeroSection.jsx — Homepage hero + categories + trust strip
  ...
  return (
    <div>
      ...
      <section style={heroStyles.hero}>
        <div style={heroStyles.heroInner} className="hero-inner">
          <div style={heroStyles.heroContent}>
            <div style={heroStyles.eyebrow}>Trusted by teachers across Australia &amp; the UK 🌱</div>
            <h1 style={heroStyles.h1} className="hero-h1">Ready-to-teach resources<br />for primary classrooms</h1>
            <p style={heroStyles.sub}>Browse hundreds of curriculum-aligned worksheets, lesson plans and activity packs — download instantly after purchase.</p>
            <div style={heroStyles.ctaRow} className="hero-cta-row">
              <button style={heroStyles.ctaPrimary} onClick={onBrowse}>Browse resources</button>
              <button style={heroStyles.ctaSecondary} onClick={onBrowse}>See free resources →</button>
            </div>
          </div>
          <div style={heroStyles.heroIllustration} className="hero-illustration">
            <div style={heroStyles.illustrationGrid}>
              {[
                ...
                <div key={i} style={{ ...heroStyles.miniCard, background: c.bg }}>
        ...
        {TRUST.map((t, i) => (
          <React.Fragment key={i}>
            <div style={heroStyles.trustItem}>
              <span style={heroStyles.trustIcon}>{t.icon}</span>
              <span style={heroStyles.trustText}>{t.text}</span>
            </div>
            {i < TRUST.length - 1 && <div style={heroStyles.trustDivider} />}
          </React.Fragment>
        ))}
      </div>
      ...
      <section style={heroStyles.section}>
        <div style={heroStyles.sectionInner}>
          <h2 style={heroStyles.sectionTitle}>Browse by subject</h2>
          <div style={heroStyles.catGrid} className="cat-grid">
            {CATEGORIES.map(cat => (
              <button key={cat.label} onClick={onBrowse} style={{ ...heroStyles.catCard, background: cat.bg }}>
                <div style={{ ...heroStyles.catIcon, color: cat.color }}>{cat.icon}</div>
                <div style={{ ...heroStyles.catLabel, color: cat.color }}>{cat.label}</div>
              </button>
            ))}
          </div>
        </div>
...
const heroStyles = {
  hero: {
    background: 'linear-gradient(160deg, #E8F5F3 0%, #FDFAF6 60%)',
    padding: '64px 0 48px',
  },
  heroInner: {
    maxWidth: '1280px',
...
Object.assign(window, { HeroSection, CATEGORIES });
