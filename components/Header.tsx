...
export default function Header() {
  ...
  const handleSearch = (e: FormEvent) => {
    e.preventDefault();
    if (searchVal.trim()) router.push(`/browse?q=${encodeURIComponent(searchVal.trim())}`);
  };
    ...
    <header style={s.nav}>
      <div style={s.inner}>
        <Link href="/" style={s.logo}>
          ...
          <span style={s.logoText}>Sprout</span>
        </Link>

        <form onSubmit={handleSearch} style={s.searchForm} className="nav-search">
...
const s: Record<string, CSSProperties> = {
  ...
  inner: { maxWidth: "1280px", margin: "0 auto", padding: "0 32px", display: "flex", alignItems: "center", gap: "20px", width: "100%" },
  logo: { display: "flex", alignItems: "center", gap: "10px", textDecoration: "none", flexShrink: 0 },
  logoMark: { display: "flex" },
  logoText: { fontFamily: "'Quicksand', 'Nunito', sans-serif", fontSize: "22px", fontWeight: 700, color: "#1A1714", whiteSpace: "nowrap", letterSpacing: "-0.01em" },
  searchForm: { flex: 1, maxWidth: "440px" },
  searchWrap: { position: "relative" },
  searchIcon: { position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", color: "#9E958A", display: "flex" },
  searchInput: {
    width: "100%", padding: "9px 12px 9px 38px", border: "1.5px solid #D6CFC6", borderRadius: "9999px",
  ...
  navGetStarted: {},
  cartBtn: {
    width: "40px", height: "40px", borderRadius: "9999px", border: "1.5px solid #D6CFC6", background: "transparent",
    cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", color: "#635C55",
  },
  cartBadge: {
    position: "absolute", top: "-3px", right: "-3px", background: "#F4A261", color: "#fff",
    borderRadius: "9999px", width: "16px", height: "16px", fontSize: "10px", fontWeight: 700,
