      ...
      <div style={s.page}>
        <div style={{ maxWidth: "600px", margin: "0 auto", padding: "80px 32px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "15px", color: "#635C55", fontFamily: "'DM Sans', sans-serif", lineHeight: 1.6, marginBottom: "28px" }}>
            <strong>{form.title || "Your resource"}</strong> is now live in the Sprout shop.
          </p>
          <div style={{ background: "#F7F3EE", borderRadius: "16px", padding: "20px", marginBottom: "28px", display: "flex", gap: "24px", justifyContent: "center" }}>
            {[
              ["Price", form.isFree ? "Free" : `$${form.price} AUD`],
              ["Type", form.type || "—"],
              ["Year group", form.yearFrom ? `${form.yearFrom}${form.yearTo && form.yearTo !== form.yearFrom ? ` – ${form.yearTo}` : ""}` : "—"],
            ].map(([l, v]) => (
          ...
          <div style={{ display: "flex", gap: "10px", justifyContent: "center" }}>
            <button style={s.btnPrimary} onClick={() => router.push("/browse")}>View in shop</button>
            <button style={s.btnOutline} onClick={() => { setPublished(false); setStep(0); setForm(EMPTY_FORM); }}>
              Upload another
            </button>
          </div>
        </div>
        ...
        <div style={s.panel}>
          ...
          {step === 2 && (
            <div style={s.stepBody}>
              ...
              <p style={s.stepSub}>Upload your PDF resource. You can also add a preview file (pages 1–2) that buyers see before purchasing.</p>
              <div style={s.dropZone} onClick={() => set("fileName", "Year_3_Multiplication_Worksheets.pdf")}>
                {form.fileName ? (
              ...
              <div style={{ display: "flex", gap: "12px", marginTop: "16px" }}>
                ...
                <div style={{ flex: 1 }}>
                  <label style={s.label}>Preview pages (optional)</label>
          ...
          <div style={s.navRow}>
            ...
            {step < UPLOAD_STEPS.length - 1 ? (
              <button style={s.btnPrimary} onClick={() => setStep((s) => s + 1)}>Continue →</button>
            ) : (
              <button style={{ ...s.btnPrimary, background: "#2A9D8F" }} onClick={() => setPublished(true)}>🌱 Publish resource</button>
            )}
          </div>
        </div>
      </div>
